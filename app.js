...
function scene(){return {version:25,items,groups,guestCount,collections,spacing,sceneRatio};}
...
function applyTransform(base,change){
  const result=TableGeometry.transform(base.items,base.pivot,sceneRatio,change);
...
function position(el,item){
  const [width,ratio]=dimensions[item.type];el.style.width=width*100+'%';el.style.height=stage.clientWidth*width*ratio+'px';el.style.left=item.x*100+'%';el.style.top=item.y*100+'%';
...
function baseline(){if(!current()||!pointers.size){gesture=null;return;}gesture={...capture(),points:[...pointers.values()].map(p=>({...p})),width:stage.clientWidth,height:stage.clientHeight};}
stage.addEventListener('pointerdown',e=>{
  if(e.pointerType==='mouse'&&e.button!==0)return;const target=e.target.closest('.piece');
...
stage.addEventListener('pointermove',e=>{
  if(pan?.id===e.pointerId){canvasWindow.scrollLeft=pan.left+pan.x-e.clientX;canvasWindow.scrollTop=pan.top+pan.y-e.clientY;return;}
...
$('duplicate').addEventListener('click',()=>{
  ...
  const moved=TableGeometry.transform(source,TableGeometry.pivot(source),sceneRatio,{dx:.04,dy:.04}).items;
...
function setPhoto(blob,ratio){
  ...
  $('scene-label').textContent=blob?'YOUR TABLE':'YOUR CANVAS';$('clear-photo').disabled=!blob;fitStage();
}
async function loadPhoto(file){
  ...
  try{const ratio=await decodePhoto(file);if(request!==photoRequest)return;checkpoint();setPhoto(file,ratio);render();announce('Photo ready. Choose guests, then tap the table to edit.');}
...
$('clear-photo').addEventListener('click',()=>{checkpoint();++photoRequest;setPhoto(null,4/3);render();announce('Photo removed from this scene. Save to replace your stored scene.');});
function fitStage(){
...
function enterFocus(){if(focused)return;previousScroll=window.scrollY;focused=true;document.body.classList.add('editing');viewZoom=1.5;$('zoom').textContent='Zoom 1.5×';requestAnimationFrame(()=>{fitStage();const item=current();canvasWindow.scrollLeft=item?item.x*stage.clientWidth-canvasWindow.clientWidth/2:(canvasWindow.scrollWidth-canvasWindow.clientWidth)/2;canvasWindow.scrollTop=item?item.y*stage.clientHeight-canvasWindow.clientHeight/2:0;stage.focus({preventScroll:true});});paint();}
function exitFocus(){focused=false;pointers.clear();gesture=null;pan=null;document.body.classList.remove('editing');fitStage();window.scrollTo(0,previousScroll);}
$('done').addEventListener('click',exitFocus);
$('zoom').addEventListener('click',()=>{viewZoom=viewZoom===1?1.5:viewZoom===1.5?2:1;$('zoom').textContent='Zoom '+viewZoom+'×';fitStage();const item=current();canvasWindow.scrollLeft=item?item.x*stage.clientWidth-canvasWindow.clientWidth/2:(canvasWindow.scrollWidth-canvasWindow.clientWidth)/2;canvasWindow.scrollTop=item?item.y*stage.clientHeight-canvasWindow.clientHeight/2:(canvasWindow.scrollHeight-canvasWindow.clientHeight)/2;});
...
function autoSet(count){
  ...
  for(const seat of TableGeometry.layout(count,sceneRatio,spacing)){
...
for(const mode of ['compact','standard','formal'])$('spacing-'+mode).addEventListener('click',()=>{
  ...
  for(const group of targets){const list=items.filter(i=>i.groupId===group.id),spread=TableGeometry.spread(list,sceneRatio,group.spacing,mode),map=new Map(spread.map(i=>[i.id,i]));items=items.map(i=>map.get(i.id)||i);group.spacing=mode;}
...
function applyScene(data){({items,groups,guestCount,collections,spacing,sceneRatio}=JSON.parse(JSON.stringify(data)));selected=null;serial=Math.max(serial,0,...items.map(i=>i.id));groupSerial=Math.max(groupSerial,0,...groups.map(g=>Number(g.id.slice(1))));pointers.clear();gesture=null;}
function validateScene(data){
  if(!data||data.version!==25||![null,2,4,6].includes(data.guestCount)||!Array.isArray(data.items)||data.items.length>500||!Array.isArray(data.groups)||data.groups.length>500||!Number.isFinite(data.sceneRatio)||data.sceneRatio<.05||data.sceneRatio>20||!Object.hasOwn(TableGeometry.spacingFactors,data.spacing))throw Error('Invalid scene');
