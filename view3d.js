...
let renderer3D=null,view3D='loading',requested3D=true,loading3D=null,measuring3D=false;
...
function update3DControls(){
 ...
 $('surface-view').hidden=is3D();$('perf-start').disabled=!is3D()||measuring3D;
...
function load3D(){
 ...
 loading3D=new Promise((resolve,reject)=>{if(window.Table3D){resolve();return;}const script=document.createElement('script');script.src='./dist/table3d.js?v=0.40.2';script.onload=resolve;script.onerror=()=>reject(Error('load'));document.head.append(script);});
 update3DControls();
 loading3D.then(()=>{
...
function sync3D(){
 ...
 $('render-status').textContent=calibration?'3D · Matched to your table':'3D · Calibrate for a table match';
...
function setup3D(){
 ...
 $('perf-start').addEventListener('click',async()=>{if(!is3D()||measuring3D)return;measuring3D=true;$('perf-start').disabled=true;$('perf-output').textContent='Measuring 3 seconds of rendering…';const result=await renderer3D.benchmark();measuring3D=false;$('perf-output').textContent=result.status==='cancelled'?'Measurement stopped. Return to 3D and measure again.':`${result.fps??'—'} FPS · ${result.cpuFrameMs} ms CPU/frame · ${result.triangles.toLocaleString()} triangles · ${(result.estimatedGraphicsBytes/1048576).toFixed(1)} MB estimated graphics allocations. ${result.jsHeapBytes?'JS heap '+(result.jsHeapBytes/1048576).toFixed(1)+' MB.':'Browser does not expose JS heap memory.'} This measures this browser, not an iPhone certification.`;$('perf-output').dataset.result=JSON.stringify(result);$('perf-start').disabled=!is3D()||measuring3D;});
 update3DControls();
}
