...
const TableCamera=(()=>{
 ...
 function solve(q,imageRatio){
  const quad=q||[{x:0,y:1},{x:1,y:1},{x:1,y:0},{x:0,y:0}],h=P.matrix(quad);
  ...
  const a=lift(h[0],h[3],h[6]),rawDepth=lift(h[1],h[4],h[7]);
  // The camera-space axis lengths, not photographed edge lengths, estimate
  // width/depth. This removes the old screen-ratio stretch from circular meshes.
  const ratio=Math.max(.2,Math.min(5,norm(a)/norm(rawDepth))),b=rawDepth.map(v=>v*ratio),t=lift(h[2],h[5],h[8]);
