...
export function create({canvas,onFailure,onFrame}){
 ...
 function stats(){const mean=xs=>xs.length?xs.reduce((a,b)=>a+b,0)/xs.length:0;let geometryBytes=0;for(const g of Object.values(geometries)){for(const a of Object.values(g.attributes))geometryBytes+=a.array.byteLength;geometryBytes+=g.index?.array.byteLength||0;}
  const pixels=canvas.width*canvas.height,environmentBytes=env.width*env.height*8,shadowBytes=light.shadow.map?light.shadow.map.width*light.shadow.map.height*8:0;return {technology:'Three.js r185 / WebGL2',...health.stats(),quality,frames,cpuFrameMs:+mean(cpu).toFixed(2),cpuScope:'render and overlay; excludes input handlers and GPU time',drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,geometries:renderer.info.memory.geometries,textures:renderer.info.memory.textures,materials:Object.values(materials).reduce((n,g)=>n+Object.keys(g).length,2),programs:renderer.info.programs?.length??null,pixelRatio:renderer.getPixelRatio(),geometryBytes,environmentBytes,environmentSize:[env.width,env.height],shadowBytes,estimatedGraphicsBytes:geometryBytes+pixels*12+environmentBytes+shadowBytes+64*64*4,memoryEstimateScope:'Geometry, packed half-float environment, contact texture, shadow color/depth and single-sample canvas buffers; excludes MSAA/driver overhead and decoded photo',jsHeapBytes:performance.memory?.usedJSHeapSize??null,items:nodes.size,groups:roots.size,assets:Object.fromEntries(Object.entries(geometries).map(([k,g])=>[k,{triangles:(g.index?.count||g.attributes.position.count)/3,vertices:g.attributes.position.count}]))};}
