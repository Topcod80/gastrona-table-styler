...
(async()=>{
 ...
 // CSS serializes pixel positions with finite precision; viewport sizes differ after reload.
 // Exact persisted scene equality is covered separately in interaction.cjs.
 const sameRendered=(actual,expected)=>{assert.equal(actual.length,expected.length);actual.forEach((p,i)=>{for(const key of Object.keys(p)){if(typeof p[key]==='number')assert.ok(Math.abs(p[key]-expected[i][key])<1e-5,`Restored ${key} differs for ${p.id}`);else assert.equal(p[key],expected[i][key]);}});};
 ...
 for(const [name,q] of Object.entries(shapes)){
  ...
  await page.reload();await ready();assert.match(await page.locator('#table-photo').getAttribute('src'),/^blob:/);sameRendered(coords(await pieces()),saved);assert.equal(await page.locator('#save-indicator').textContent(),'Saved on this device');
  ...
  await page.locator('#edit-guests').selectOption('2');assert.equal(await page.locator('#save-indicator').textContent(),'Unsaved changes');await tap('tab-save');await tap('edit-restore');await page.waitForFunction(()=>document.querySelectorAll('.piece').length===24);sameRendered(coords(await pieces()),saved);await tap('tab-arrange');
